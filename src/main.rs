fn main() {
    // println!("Solution to euler1: {}", euler1(1000));
    // println!("Solution to euler2: {}", euler2(4000000));
...
fn is_prime(number: u64) -> bool {
    ...
    for i in 2..1 + ((number as f64).sqrt().ceil() as u64) {
        if number % i == 0 {
            return false;
        };
    }
...
fn all_same(vector: &Vec<u64>) -> bool {
    let value = vector[0];
...
fn euler3(mut number: u64) -> u64 {
    let mut i: u64 = 2;
    let mut max_factor: u64 = 0;

    while i <= ((number as f64).sqrt().ceil() as u64) {
...
fn euler4(number_of_digits: u32) -> u32 {
    let mut palindrome = 0;
    let upper_limit = 10_u32.pow(number_of_digits);

    for i in 100..upper_limit {
        for j in 100..upper_limit {
            let test_string = (i * j).to_string();
            if test_string == test_string.chars().rev().collect::<String>() {
                if i * j > palindrome {
                    palindrome = i * j;
                }
            }
...
fn euler5(upper_limit: u64) -> u64 {
    ...
    for i in base + 1..1 + upper_limit {
        numbers.push(i);
    }

    let upper_limit = upper_limit - base;

    for i in 0..upper_limit as usize {
        print!("{}, ", numbers[i]);
    }
    println!("");
    ...
    loop {
        ...
        let max_value: u64 = match max_value {
            Some(max) => *max,
            None => 0,
        };
...
fn euler7(nth_number: u64) -> u64 {
    let mut n: u64 = 2;
    let mut nth_prime: u64 = 1;

    while nth_prime <= nth_number {
        if is_prime(n) {
            nth_prime += 1;
        }
        n += 1;
...
fn euler8() -> u64 {
    ...
    for i in 0..thousand_digits.len() - 13 {
        let mut this_product: u64 = 1;
        let thirteen_digits = &thousand_digits[i..i + 13];
        for n in thirteen_digits.chars() {
            this_product *= n.to_digit(10).unwrap_or(0) as u64;
            if this_product == 0 {
                break;
            }
        }
...
fn euler10(upper_limit: u64) -> u64 {
    let mut sum: u64 = 0;
...
fn euler11() -> u32 {
    ...
    for y in 1..20 as usize {
        for x in 1..20 as usize {
            if y <= 16 {
                let product =
                    grid[y][x] * grid[y + 1_usize][x] * grid[y + 2_usize][x] * grid[y + 3_usize][x];
            ...
            if x <= 16 {
                let product =
                    grid[y][x] * grid[y][x + 1_usize] * grid[y][x + 2_usize] * grid[y][x + 3_usize];
                if product > max_product {
                    max_product = product;
                }
            }
            if y <= 16 && x <= 16 {
                let product = grid[y][x]
                    * grid[y + 1_usize][x + 1_usize]
                    * grid[y + 2_usize][x + 2_usize]
                    * grid[y + 3_usize][x + 3_usize];
                if product > max_product {
                    max_product = product;
                }
            }
...
fn euler12(min_divisors: u64) -> u64 {
